...
gcm_dat_path <- "input_data/"
lm_dat_path <- "input_data/rothc_lm_in_dat.xlsx"
out_dat_path <- "sim_data/"
...
pop_gis_dat_ls <- function(gis_dat_path) {
  
  # load gis data file row of interest
  gis_dat_row_means <- read_xlsx(gis_dat_path) %>%
    select(!cell_id) %>%
    filter(!is.na(gldas_dec)) %>%
    summarise_at(vars(!starts_with("nlcd") & !contains("total")), mean)
  
  gis_dat_row_sums <- read_xlsx(gis_dat_path) %>%
    filter(!is.na(gldas_dec)) %>%
    summarise_at(vars(starts_with("nlcd") | contains("total")), sum, na.rm=TRUE)
  
  gis_dat_row <- bind_cols(gis_dat_row_means, gis_dat_row_sums)
...
gis_dat_ls <- pop_gis_dat_ls(params_ls$gis_dat_path)


## Import EPSCoR GCM climate data
pop_gcm_dat_ls <- function(gcm) {
  
  gcm_dat <- read_csv(paste(params_ls$gcm_dat_path, gcm, "_1950-2099_sa_avg.csv", sep = ""), show_col_types = F)
...
gcm_dat_ls <- pop_gcm_dat_ls(params_ls$gcm)
...
for (lu_lab in params_ls$lu_labs) {
  ...
  if (bgc_opt$objective > 1) {
    print(paste("Warning: ", lu_lab, ", did not converge during spinup: diff = ", round(bgc_opt$objective, digits = 2), sep = ""))
  }
...
write_csv(out_dat, file = paste(out_dat_path, "sim_data_", params_ls$gcm, "_full_sa.csv", sep = ""))
